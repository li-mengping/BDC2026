...
from code.utils.data_manifest import build_data_manifest


START_DATE = "2023-01-02"
END_DATE = "2026-06-28"
EXPECTED_FIRST_TRADING_DATE = "2023-01-03"
EXPECTED_LAST_TRADING_DATE = "2026-06-26"
...
def get_stock_history(bs_code, start_date, end_date):
    ...
    df['date'] = dates.map(lambda value: f'{value.year}/{value.month}/{value.day}')
...
def load_or_fetch_hs300_stocks(path: str, refresh: bool = False) -> pd.DataFrame:
    """优先复用已保存成分股，只有显式刷新时才查询当前成分股。"""
    if os.path.exists(path) and not refresh:
        frozen = pd.read_csv(path, dtype={'code': str})
        if 'code' not in frozen.columns:
            raise ValueError(f'冻结成分股缺少 code 列: {path}')
        return frozen
...
def main():
    ...
    try:
        # 获取沪深300成分股
        hs300_df = load_or_fetch_hs300_stocks(hs300_list_path, refresh=args.refresh_constituents)
        ...
        if existing_df is not None and not existing_df.empty:
            range_frame = existing_df[['股票代码', '日期']].copy()
            range_frame['股票代码'] = range_frame['股票代码'].astype(str).str.zfill(6)
            range_frame['日期'] = pd.to_datetime(range_frame['日期'], errors='coerce')
            if range_frame['日期'].isna().any():
                raise ValueError('现有数据包含无效日期，无法安全续传')
            grouped = range_frame.groupby('股票代码', sort=False)['日期'].agg(['min', 'max'])
            existing_ranges = {
                stock_id: (str(values['min'].date()), str(values['max'].date()))
                for stock_id, values in grouped.iterrows()
            }
        ...
        for idx, row in hs300_df.iterrows():
            ...
            # 检查该股票是否已存在数据
            existing_min_date, existing_max_date = existing_ranges.get(pure_code, (None, None))

            if existing_min_date and existing_max_date:
                # 已有数据，检查是否需要增量
                need_early = existing_min_date > EXPECTED_FIRST_TRADING_DATE
                need_late = existing_max_date < EXPECTED_LAST_TRADING_DATE
        ...
        if failed_stocks:
            ...
            raise RuntimeError(f'数据下载存在失败股票: {len(failed_stocks)}')
        ...
        if os.path.exists(output_path):
            ...
            final_df = final_df.sort_values(['股票代码', '日期']).reset_index(drop=True)
            final_df.to_csv(output_path, index=False, encoding='utf-8-sig')
            build_data_manifest(
                output_path,
                hs300_list_path,
                os.path.join(save_dir, 'manifest.json'),
                source='baostock frozen hs300 constituent snapshot',
            )
            print('数据 manifest 已生成: data/manifest.json')
